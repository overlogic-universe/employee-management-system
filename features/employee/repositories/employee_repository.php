...
class EmployeeRepository
{
    ...
    {
        $qrContent = "{$employeeId}-{$recipientEmail}-{$divisionId}";
        $qrCode = "https://api.qrserver.com/v1/create-qr-code/?data=" . urlencode($qrContent) . "&size=150x150";
        $emailBody = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Attendance QR Code</title><style>body{font-family:\'Segoe UI\',Tahoma,Geneva,Verdana,sans-serif;background:linear-gradient(135deg,#041433 0%,#1e3a6d 100%);color:#fff;margin:0;padding:0;display:flex;justify-content:center;align-items:center;min-height:100vh}.container{max-width:600px;padding:40px;background:linear-gradient(45deg,rgba(255,255,255,0.1),rgba(255,255,255,0.2));border-radius:30px;box-shadow:0 8px 32px rgba(0,0,0,0.1);backdrop-filter:blur(10px);}h2{color:##3d65b0;text-align:center;margin-bottom:30px;font-size:28px;text-shadow:2px 2px 4px rgba(0,0,0,0.3)}p{text-align:center;line-height:1.6;margin-bottom:20px}.qr-code{display:flex;align-items:center;justify-content:center;text-align:center;margin-bottom:30px;padding:20px;background:linear-gradient(45deg,#3d65b0,#1e3a6d);border-radius:20px;box-shadow:0 4px 15px rgba(0,0,0,0.2)}.qr-code img{max-width:100%;height:auto}.date{font-weight:700;color:##3d65b0}</style></head><body><div class="container"><h2>Welcome, ' . htmlspecialchars($name) . '!</h2><p>Here\'s your attendance QR code for <span class="date">' . htmlspecialchars(date('Y-m-d')) . '</span>:</p><div class="qr-code"><img src="' . $qrCode . '" alt="Attendance QR Code"></div><p>Scan this QR code to mark your attendance.</p></div></body></html>';
